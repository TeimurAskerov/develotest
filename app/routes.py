...
def card():
    ...
    if card_form.validate_on_submit():
        ...
        if card is None:
            return render_template('fail.html', message='Неверный номер карты', url=url_for('card'))
        elif not card.active:
            return render_template('fail.html', message='Карта заблокирована', url=url_for('card'))
        ...
        # return render_template('pin.html', form=pin_form, token_hex=token_hex)
...
def pin(token_hex):
    ...
    card = token.get_card()
    if card is None:
        return render_template('fail.html', message='Недействительная карта', url=url_for('card'))
    elif not card.active:
        return render_template('fail.html', message='Карта заблокирована', url=url_for('card'))

    pin_form = PinForm()
    if pin_form.validate_on_submit():
        if (card is not None and card.active and card.tries <= 3
                and card.check_pin_code(pin_form.pin.data)):
            card.tries = 0
            token.active = True
            db.session.commit()
            return redirect(url_for('options', token_hex=token_hex))
            # return render_template('options.html', token_hex=token_hex, form=opt_form)
        ...
        if not card.check_pin_code(pin_form.pin.data):
            ...
            if card.tries > 3:
                card.active = False
                card.tries = 0
                card.add_history('666')
                db.session.commit()
                message = 'Превышено количество попыток ввода кода. Карта заблокирована'
...
def balance(token_hex):
    ...
    card = token.get_card()
    if card is None:
        return render_template('fail.html', message='Недействительная карта',
                               url=url_for('balance', token_hex=token_hex))

    card.add_history('111')
    ...
    if cash_form.validate_on_submit():
        if cash_form.back.data:
            ...
            if card.amount < amount:
                return render_template('fail.html', message='Недостаточно денег на счету',
                                       url=url_for('cash', token_hex=token_hex))

            card.amount = round(card.amount - amount, 2)
            card.add_history('222', {'amount': amount})
            db.session.commit()
            bill_form = BillForm()
